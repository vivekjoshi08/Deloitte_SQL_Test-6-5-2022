...
where p.dno=d.dno and d.mgr_ssn=e.ssn and e.lname='SCOTT'
union
...
from employee e
where not exists( select pno from project where dno=5 not in (select pno from works_on where e.ssn =ssn));

#5
select e.dno, count(e.ssn) as 'Number of Employees'
from employee e where e.salary> 600000 and e.dno in (select e1.dno from employee e1 group by e1.dno
